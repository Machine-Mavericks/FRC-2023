...
public class AutoDriveToPose extends CommandBase {
  ...
  public void initialize() {
    ...
    if (m_recallPoint)
      m_target = RobotContainer.odometry.RecallPoint(0); // replace odometry with swerveodometry
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    Pose2d curr = RobotContainer.odometry.getPose2d();// replace odometry with swerveodometry
    ...
    RobotContainer.swervedrive.drive(new Translation2d(xSpeed*RobotContainer.swervedrive.MAX_VELOCITY_METERS_PER_SECOND,
                                                      ySpeed*RobotContainer.swervedrive.MAX_VELOCITY_METERS_PER_SECOND),
                                    rotSpeed*RobotContainer.swervedrive.MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND,
                                    true);
  }
